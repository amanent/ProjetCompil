...
/* retrouve la valeur d'une variable stockee dans 'tree' si elle apparait
...
int eval(TreeP tree, VarDeclP decls) {
  switch (tree->op) {
  case ID: return evalVar(tree, decls);
  case CST: return (tree->u.val);
  case EQ: return eval(getChild(tree, 0), decls) == eval(getChild(tree, 1), decls);
  case NE: return eval(getChild(tree, 0), decls) != eval(getChild(tree, 1), decls);
  case GT: return eval(getChild(tree, 0), decls) > eval(getChild(tree, 1), decls);
  case GE: return eval(getChild(tree, 0), decls) >= eval(getChild(tree, 1), decls);
  case LT: return eval(getChild(tree, 0), decls) < eval(getChild(tree, 1), decls);
  case LE: return eval(getChild(tree, 0), decls) <= eval(getChild(tree, 1), decls);
  case IF: 	if(eval(getChild(tree, 0), decls))
  				return eval(getChild(tree, 1), decls);
  			else
  				return eval(getChild(tree, 2), decls);
  case PUT: return putValue(getChild(tree, 0)->u.str, eval(getChild(tree, 1), decls));
  case GET: return getValue();
  case ADD: return eval(getChild(tree, 0), decls) + eval(getChild(tree, 1), decls);
  case SUB: return eval(getChild(tree, 0), decls) - eval(getChild(tree, 1), decls);
  case MUL: return eval(getChild(tree, 0), decls) * eval(getChild(tree, 1), decls);
  case DIV: return eval(getChild(tree, 0), decls) / eval(getChild(tree, 1), decls);
  default: 
